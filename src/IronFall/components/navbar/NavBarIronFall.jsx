...
import logo from '../../../assets/TassoIronFAll.png';
...
export const NavBarIronFall = () => {
    ...
    const navLinks = [
        { name: 'Auctions', path: '/app.ironfall/auction' },
        { name: 'Manager Auction', path: '/app.ironfall/manager' },
        { name: 'New Auction', path: '/app.ironfall/newauction' },
    ];
        ...
        <div className="flex space-x-7 font-semibold">
            ...
            {navLinks.map((link) => (
                <Link
                    key={link.name}
                    to={link.path}
                    className={`text-lg px-3 py-2 rounded-md transition duration-300 ${location.pathname === link.path ? 'text-red-600' : 'hover:text-red-400 text-gray-100'
                        }`}
                >
                    {link.name}
                </Link>
