...
import { setApprovalPact } from "../../BlockchainOperation/IronPactOp";
import { useEthersSigner } from "../../helper/ClientToSigner";

// eslint-disable-next-line react/prop-types
export const AuthorizePact = ({ contractAddress, setChange }) => {
    ...
    const authSpending = async (e) => {
        ...
        setIsLoading(true);
        try {
            const tx = await setApprovalPact(contractAddress, true, signer)
    ...
    return (
        <div className="relative w-full md:w-2/5 bg-gradient-to-b from-slate-900 to-gray-800 text-white rounded-xl p-6 shadow-xl">
            ...
            <button
                onClick={setChange}
                className="absolute top-2 right-2 md:top-4 md:right-4 w-8 h-8 flex items-center justify-center rounded-full text-gray-400 hover:text-white focus:outline-none hover:bg-red-600 transition-all">
                &times;
            </button>
            ...
            <div className="text-center mb-6">
                <h3 className="font-extrabold text-2xl bg-gradient-to-r from-blue-400 via-blue-500 to-blue-600 text-transparent bg-clip-text">
                    Authorize Pact Spending
                </h3>
                <p className="text-sm text-gray-400">Securely authorize the contract for Pact usage.</p>
            </div>
